...
def compound_combination(category_name, non_heads):
    for head in heads:
        for non_head in non_heads:
            ...
            print(surprisal_values)  # Debug

            token_count = len(surprisal_values)

            if len(surprisal_values) == 5:
                surprisal_non_head = sum(surprisal_values[1:3])
                surprisal_head = sum(surprisal_values[3:5])  
            elif len(surprisal_values) == 4:
                surprisal_non_head = surprisal_values[1]
            ...
            print(f"{sentence}: Non-Head: {surprisal_non_head}, Head: {surprisal_head}")  # Debug
...
output_file = "results_babble.csv"
df = pd.DataFrame(data, columns=["Category", "Non-Head", "Head", "Surprisal Non-head", "Surprisal head"])
df.to_csv(output_file, index=False)

print('\n results in results_babble.csv \n')